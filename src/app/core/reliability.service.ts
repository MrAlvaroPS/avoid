...
// attended_percentage). Preparación = solo encantamientos por ahora
// (7 slots universalmente encantables) — las gemas SÍ están disponibles
// (equipped_items[].gems[]) pero sin dato de cuántos sockets tiene cada
// ítem no hay umbral verificable que puntuar, así que quedan fuera de la
// fórmula (gem_count se trae igualmente, informativo). Nunca se rellena un
// eje ausente con un cero silencioso — se excluye y se renormaliza.
...
export interface PlayerReliability {
  ...
  /** §"gem_count se calcula pero no se muestra en ninguna pantalla": gemas engarzadas en el pull más reciente de la ventana — informativo, no puntuado (sin dato de sockets máximos por ítem). null = sin ningún pull con equipped_items en la ventana. */
...
export function computeReliabilityBreakdown(rows: ReliabilityInputRow[], now: number, asistenciaPct: number | null = null): ReliabilityBreakdown | null {
  ...
  for (const r of rows) {
    ...
    if (r.used_defensive_when_died != null) {
      defSum += (r.used_defensive_when_died ? 1 : 0) * w;
      defWeight += w;
    }
    if (r.enchantable_slot_count > 0) {
      prepSum += (r.enchanted_slot_count / r.enchantable_slot_count) * w;
      prepWeight += w;
    }
  }
  ...
  return { overall, breakdown: { mecanica, defensiva, preparacion, asistencia: asistenciaPct } };
}
...
export class ReliabilityService {
  ...
  async listPlayerReliability(scope?: { bossId: string; difficulty: string }): Promise<PlayerReliability[]> {
    let query = this.supabase.client
      ...
      .select('player_name, closed_at, had_avoidable_damage, self_positioning_death, used_defensive_when_died, enchanted_slot_count, enchantable_slot_count, gem_count');
    query = scope ? query.eq('boss_id', scope.bossId).eq('difficulty', scope.difficulty) : query.gte('closed_at', new Date(Date.now() - WINDOW_DAYS * 86_400_000).toISOString());

    const [inputsRes, roster, realAttendance] = await Promise.all([
      query,
    ...
    for (const [playerName, rows] of byPlayer) {
      // §"gem_count se calcula y guarda pero no se muestra en ninguna
      // pantalla" (feedback real): informativo, no puntuado (sin dato de
      // sockets máximos por ítem, no hay umbral real que puntuar sin
      // inventarlo — igual que documenta ya la vista SQL). Se enseña el del
      // pull MÁS RECIENTE de la ventana, no una media — es "cuántas gemas
      // tiene puestas ahora", no un acumulado histórico sin sentido para gear.
  ...
  async getNightReliability(reportCode: string, playerName: string): Promise<ReliabilityBreakdown & { sampleSize: number }> {
    ...
    if (!pullIds.length) return { overall: 0, breakdown: { mecanica: null, defensiva: null, preparacion: null, asistencia: null }, sampleSize: 0 };
    ...
    const { data } = await client
      ...
      .in('pull_id', pullIds);
    const rows = (data ?? []) as ReliabilityInputRow[];
    const result = computeReliabilityBreakdown(rows, Date.now());
    return result ? { ...result, sampleSize: rows.length } : { overall: 0, breakdown: { mecanica: null, defensiva: null, preparacion: null, asistencia: null }, sampleSize: 0 };
  }
}
