-- Durable automatic refresh of the published canonical defensive generation.
--
...
begin
  ...
  return true;
exception when others then
  -- Wake-up is best-effort. Queue state is the durable source of truth.
  return false;
end;
$$;
...
begin
  ...
  -- A successful worker completion atomically moved the published pointer.
  update canonical_defensive_refresh_requests q
  set status = 'completed',
      completed_at = now(),
      lease_token = null,
...
-- Backfill already-complete reports (including manually imported raids) that
-- arrived after the current published generation.
insert into canonical_defensive_refresh_requests (
