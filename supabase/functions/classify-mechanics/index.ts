...
function buildSystemPrompt(bossName: string, difficulties: string[]): string {
  ...
  return `Eres un investigador experto en encuentros de raid de World of Warcraft. HOY es ${todayIso()} — contrasta cada dato contra el PARCHE/TUNING VIGENTE HOY, nunca contra una versión anterior del encuentro que puedas recordar de tu entrenamiento (una mecánica puede haberse ajustado, quitado, o cambiado de umbral en un hotfix; una guía vieja que ya no aplica falsea los datos tanto como una inventada — §"hay varios que son viejos y están falseando datos", feedback real). Si una fuente no deja claro de qué parche/hotfix es, prioriza la más reciente y dilo en "notes". Tu tarea es clasificar habilidades del boss "${bossName}" en una de 10 categorías, investigando en TODAS las fuentes reales que tengas disponibles, A DÍA DE HOY (Wowhead —tooltip, comentarios y Dungeon Journal—, Icy Veins, guías de Method/Wowhead, Warcraft Logs, vídeos de guía si puedes acceder a su contenido, foros, etc.). Busca por el NOMBRE de la habilidad (en español o en inglés, lo que te dé mejores resultados) — el ability_id solo sirve para identificarla en tu respuesta, no suele ser buscable.
