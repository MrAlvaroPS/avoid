...
const RUN_KIND = 'e6_e7_shadow_v7_two_log';
const REPORT_CODES = ['7GbANtw1J2pjZzH9', '24nwKPpL8tr9Bcg1'] as const;
const PREDECESSOR_GENERATION_ID = '32505e86-185d-4f28-b7dc-225afd2554fa';
...
async function start(client: any) {
  ...
  if (pointer?.published_generation_id != null) throw new Error('Shadow v7 precondition: published pointer must remain null.');
  if (pending !== 0) throw new Error(`Shadow v7 precondition: pending semantics=${pending}, expected 0.`);
  ...
  if (scopedPulls !== 26) throw new Error(`Shadow v7 scope drift: expected 26 canonical pulls across two reports, got ${scopedPulls}.`);
  ...
  const notes = JSON.stringify({
    ...
    versions: {
      resolver: EFFECTIVE_DEFENSIVE_RESOLVER_VERSION_V7,
      semanticResolver: EFFECTIVE_DEFENSIVE_SEMANTIC_RESOLVER_VERSION_V7,
      evaluator: DEFENSIVE_EPISODE_EVALUATOR_VERSION_V7,
    },
  ...
  const { data, error } = await client.from('defensive_generations').insert({
    ...
    resolver_version: EFFECTIVE_DEFENSIVE_RESOLVER_VERSION_V7,
    semantic_resolver_version: EFFECTIVE_DEFENSIVE_SEMANTIC_RESOLVER_VERSION_V7,
    episode_version: DEFENSIVE_EPISODE_EVALUATOR_VERSION_V7,
    evaluator_version: DEFENSIVE_EPISODE_EVALUATOR_VERSION_V7,
...
async function processOne(client: any, generationId: string) {
  ...
  if (gen.resolver_version !== EFFECTIVE_DEFENSIVE_RESOLVER_VERSION_V7 ||
      gen.semantic_resolver_version !== EFFECTIVE_DEFENSIVE_SEMANTIC_RESOLVER_VERSION_V7 ||
      gen.episode_version !== DEFENSIVE_EPISODE_EVALUATOR_VERSION_V7) throw new Error('Generation version drift.');
  ...
  try {
    ...
    if (hard.length) throw new Error(`v7 hard gate violation: ${JSON.stringify(hard.slice(0, 10))}`);
